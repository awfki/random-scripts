...
function UpdateScript()
{
  ...
  if [ "$NewScriptAvailable" = "t" ]; then
    ...
    if [ "$Writable" = "yes" ]; then
      ...
      /bin/chmod 755 "$DirName"/"$ScriptName"
...
MY_IP_ADDRESS="$(/sbin/ifconfig $DEFAULT_INTERFACE | grep "inet " | awk '{ print $2 }')"
...
# Print a warning if you run older than 10.8
if [ $(sw_vers -productVersion | cut -d\. -f2) -lt 8 ]; then
  printf "${ESC}${BlackBack};${YellowFont}mYour Operating System is old ($(/usr/bin/sw_vers -productVersion)). Beware that this script may not work correctly on your system${Reset}\n"
  echo
fi
...
if [ -z "$(echo "$SW_VERS" | grep -i server)" ]; then
  ...
  [[ -x /usr/sbin/serverinfo ]] && /usr/sbin/serverinfo --software 1>/dev/null
  # Exit code 0 = server; 1 = NOT server
  ServSoft=$?
  if [ $ServSoft -eq 0 ]; then
    # Is it configured?
    /usr/sbin/serverinfo --configured 1>/dev/null
    ServConfigured=$?
...
/usr/sbin/networksetup -listnetworkserviceorder | egrep "^\([0-9\*]*\)\ " | sed -e 's/^(//g' -e 's/) /:/' > $NIfile
# Example:
# 1:Bluetooth DUN
# 2:Ethernet 1
# 3:Ethernet 2
...
# Note that there can be problems with OS X < 10.8!

# Determine if there is a ppp-connection:
ppp="$(/sbin/ifconfig | grep "ppp[0-9]" | awk '{print $1}' | cut -d: -f1)"
# Ex: ppp=’ppp0’
# Also get the data
if [ -n "$ppp" ]; then
  # Find the ppp-address:
  ppp_temp="$(/sbin/ifconfig | grep -A1 $ppp | tail -1 | grep 'inet ' | awk '{print $2}')"
  # Ex: ppp_temp='130.235.252.33'
  ppp_mask="$(/sbin/ifconfig | grep -A1 $ppp | tail -1 | grep 'inet ' | awk '{print $6}' | cut -c3-)"
...
do
  Interface="$(/usr/sbin/networksetup -listallhardwareports 2>/dev/null | grep -A1 "Hardware Port: $IFName" | tail -1 | awk '{print $2}')"
  # Ex: en0
  MediaSpeed="$(/usr/sbin/networksetup -getMedia "$IFName" 2>/dev/null | grep "^Active" | cut -d: -f2-)"
  # Ex: "1000baseT" or "autoselect"
  Configuration="$(/usr/sbin/networksetup -getinfo "$IFName" 2>/dev/null | grep Configuration | awk '{print $1}')"
  # Ex: "DHCP" or "Manual"
  IPaddress="$(/usr/sbin/networksetup -getinfo "$IFName" 2>/dev/null | grep "^IP address" | cut -d: -f2)"
  # Ex: " 130.235.16.211"
  SubnetMask="$(/usr/sbin/networksetup -getinfo "$IFName" 2>/dev/null | grep "^Subnet mask" | cut -d: -f2)"
  # Ex: " 255.255.254.0"
  Router="$(/usr/sbin/networksetup -getinfo "$IFName" 2>/dev/null | grep "^Router" | cut -d: -f2)"
  # Ex: " 130.235.16.1"
  #MACaddress="$(networksetup -getinfo "$IFName" | grep "^Ethernet Address" | cut -d: -f2-7)"
  MACaddress="$(/sbin/ifconfig "$Interface" 2>/dev/null | grep "ether\ " | awk '{print $2}')"
